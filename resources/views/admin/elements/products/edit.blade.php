...
<div class="row" id="user-profile">
   <div class="col-lg-12">
      ...
      <div class="card">
        ...
        <form action="{{route('admin.update_product',$product->product_id)}}" method="POST" enctype="multipart/form-data">
                  @csrf
            <div class="row">
                     ...
                     <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" step="0.01" class="form-control" required 
                             name="regular_price"   id="regular_price" value="{{old('regular_price',$product->product_price_regular)}}" placeholder="Regular Price" oninput="regularPriceChange()">
                        </div>
                    </div>
                  ...
                  <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <select name="product_cat_id" required id="category" class="form-control"  >
                             ...
                             <option {{old('product_cat_id',$product->item_category_id) == $key->item_category_id ? 'selected':''}} value="{{ @$key->item_category_id }}">{{ @$key->category_name }}</option>
                             @endforeach
                        </select>
                    </div>
                 </div>
                     ...
                     <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select name="sub_category_id"  id="sub_category_id" class="form-control"  >
                                 ...
                                 <option {{old('sub_category_id',$product->item_sub_category_id) == $key->item_sub_category_id ? 'selected':''}} value="{{ @$key->item_sub_category_id }}">{{ @$key->sub_category_name }}</option>
                                 @endforeach
                            </select>
                        </div>
                     </div>
                     ...
                     <div class="col-md-4">
                      <div class="form-group">
                          ...
                          <select name="iltsc_id" id="iltsc_id" class="form-control"  >
                               ...
                               @foreach ($subcategorieslevtwo as $key)
                               <option {{old('iltsc_id',$product->iltsc_id) == $key->iltsc_id ? 'selected':''}} value="{{ @$key->iltsc_id }}">{{ @$key->iltsc_name }}</option>
                               @endforeach
                          </select>
                      </div>
                   </div>
                        ...
                        <div class="form-group">
                          <label class="form-label">Brand *</label>
                           <select required name="brand_id" id="brand_id" class="form-control"  >
                                ...
                                <option {{old('brand_id',$product->brand_id) == $key->brand_id ? 'selected':''}} value="{{$key->brand_id }}"> {{$key->brand_name }} </option>
                                @endforeach
                              </select>
                        </div>
                     </div>
                              ...
                              <thead>
                               ...
                               <tbody class="col-lg-12 col-xl-6 p-0">
                                ...
                                @if(!$product_images->isEmpty())
                                 ...
                                 <tr>
                                    <td>{{$i}}</td>
                                    <td><img src="{{asset('/assets/uploads/products/'.$product_image->item_image_name)}}"  width="50" ></td>
                                    <td>{{@$product_image->productVariant->variant_name}}</td>
                                    @php
                                      $productVariantId = $product_image->product_variant_id;
                                      if($productVariantId == null)
                                      $productVariantId = 0;

                                    @endphp
                                    <td><input type="checkbox" class="csatatus{{ $productVariantId }}" @if (@$product_image->is_default == 1) checked @endif value="1" 
                                    onchange="changeBaseImage({{$product_image->item_image_id}},{{$product_image->product_variant_id}})"  name="image_flag" 
                                    id="image_flag{{$product_image->item_image_id}}"></td>

                                    <td>
                                        <form action="{{route('admin.destroy_product_image',$product_image->item_image_id)}}" method="POST">
                                            @csrf
                                            @method('POST')
                           ...
                           <table  id="example" class="table table-striped table-bordered">
                              ...
                              @if(!$productVar->isEmpty())
                                 ...
                                 <tr>
                                    ...
                                    <td>
                                       <img src="{{asset( (new \App\Helpers\Helper)->productVarBaseImage($product->product_id,$product->product_variant_id))}}"  
                                            width="50" >
                                    </td>
                                    ...
                                    <td>
                                       ...
                                       <a  data-toggle="modal" data-target="#AddAttrModal{{$value->product_variant_id}}" class="mt-2  text-white btn btn-sm btn-yellow"> Attributes</a>
               ...
               <tbody class="col-lg-12 col-xl-6 p-0">
                  ...
                  @foreach ($var_atts as $val)
                     ...
                     <tr>
                        ...
                        <td>
                           <form action="{{route('admin.destroy_product_var_attr',$val->variant_attribute_id)}}" method="POST">
                              @csrf
                              @method('POST')
                              <button type="submit" onclick="return confirm('Do you want to delete this item?');"  class="btn btn-sm btn-danger">Delete</button>
                           </form>
    ...
    if (typeof varId === "undefined") {
      varId =  0;
            ...
            if($('#image_flag'+id).prop('checked'))
            {
                    ...
                    $.ajax({
                        type:"GET",
                        url:"{{ url('/ajax/product/set_default_image')}}?item_image_id="+id+"&product_variant_id="+varId,
